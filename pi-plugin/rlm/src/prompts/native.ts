...
function nativeReplGlossary(): string {
  return [
    ...
    "- **ALWAYS SPAWN:** `llm_query` / `llm_batch` / `rlm_query` / `rlm_batch` → `Task` immediately (never the answer).",
    "  Collect with `await_task(t)` or `await_task([t1,t2])`. Fire independent Tasks first, then await.",
    "- `llm_query(prompt) -> Task` — one sub-LLM. `llm_batch(prompts) -> Task` — many in parallel (await → list).",
    CHUNKED_GLOSSARY_LINE_NATIVE,
    "- `rlm_query(prompt, paths=None) -> Task` — recursive RLM worker. Prefer llm_query when one-shot is enough.",
    "  The child inherits your `context`. Describe the task; never paste file text. `paths=['src/auth/']` narrows.",
    "- `rlm_batch(tasks, paths=None) -> Task` — many RLM workers in parallel (await → list of reports).",
    "- `map_files` / `llm_query_chunked` / `llm_map_reduce` still block until done (helpers).",
    "- `spawn(fn, *args) -> Task` — optional; for core tools same as calling them. Not `llm_map_reduce`.",
    "",
    "",
